...
- `WORKTREE_PARENT="${REPO_ROOT%/*}/.git-worktrees"`
- `WORKTREE_PATH="$WORKTREE_PARENT/${REPO_NAME}-issue{issueNumber}"`
...
- `git worktree remove --force "$WORKTREE_PATH"` when path exists
...
- Remove issue worktree: `git worktree remove --force "$WORKTREE_PATH"`.
